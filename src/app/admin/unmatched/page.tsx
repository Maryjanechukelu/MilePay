        ...
        <div key={p.id} className="card p-4">
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={() => handleResolve(p.id, "return")} disabled={resolving === p.id} className="btn-outline btn-sm flex-1 justify-center gap-1.5 disabled:opacity-40">
              {resolving === p.id ? <span className="w-3 h-3 border border-current border-t-transparent rounded-full animate-spin" /> : <RefreshCw size={12} />} Return to sender
            </button>
            <button onClick={() => handleResolve(p.id, "match")} disabled={resolving === p.id} className="btn-primary btn-sm flex-1 justify-center gap-1.5 disabled:opacity-40">
              {resolving === p.id ? <span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <CheckCircle size={12} />} Match to project
            </button>
          </div>
        </div>
      ))}
